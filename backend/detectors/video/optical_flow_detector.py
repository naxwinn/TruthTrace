...
class OpticalFlowDetector:
    ...
    def _detect_motion_discontinuity(self):
        ...
        prev_gray = cv2.imread(str(self.frame_paths[0]), cv2.IMREAD_GRAYSCALE)
        if prev_gray is None:
            return

        for i in range(1, len(self.frame_paths)):
            curr_gray = cv2.imread(str(self.frame_paths[i]), cv2.IMREAD_GRAYSCALE)
        ...
        for idx in anomaly_indices:
            ...
            if z_score > threshold:
                ...
                self.findings.append({
                    ...
                    "start_time": round(frame_time - 0.5 / self.fps, 3),
                    "end_time": round(frame_time + 0.5 / self.fps, 3),
        ...
        for i in range(1, len(self.frame_paths)):
            ...
            if correlation > threshold_ssim:
                ...
                if static_run >= 3:  # At least 3 identical frames at 2fps = 1.5s freeze
                    start_time = static_start / self.fps
                    end_time = (static_start + static_run) / self.fps
